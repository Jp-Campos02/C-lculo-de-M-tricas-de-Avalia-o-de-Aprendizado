...
def calcular_metricas(matriz):
    ...
    acurácia = (VP + VN) / np.sum(matriz) if np.sum(matriz) > 0 else 0
...
# Cálculo das métricas para uma das matrizes de confusão
print("\nMétricas para Naive Bayes com Bag of Words:")
metricas_nb_bow = calcular_metricas(matriz_confusao_nb_bow)
for chave, valor in metricas_nb_bow.items():
